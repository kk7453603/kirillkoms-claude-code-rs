...
Практические примеры работы с Claude Code RS. Запустите агент командой `./target/release/claude-code` и введите показанный запрос.

### 1. Рефакторинг функции

**Задача:** Функция выросла и делает несколько вещей сразу.
...
Агент читает файл, выделяет логические блоки (system messages, tools, tool_choice), создаёт вспомогательные функции и обновляет файл. Предлагает запустить тесты для проверки.

### 2. Code Review существующего PR

**Задача:** Провести review открытого pull request перед мерджем.

```
/review
```

Агент загружает diff PR из GitHub, анализирует изменения на предмет ошибок, нарушений стиля и потенциальных проблем. Возвращает структурированный список замечаний с указанием файлов и строк.
...
Агент читает `streaming.rs` вокруг строки 42, находит условие при котором индекс выходит за границы, исправляет код и предлагает добавить тест на этот edge case.
...
> Изучи crates/cc-api/src/providers/openai_translate.rs и напиши unit-тесты
  для функции translate_response. Покрой happy path и edge cases:
  пустой контент, несколько tool calls, отсутствующий role.
```

Агент читает существующие тесты в файле для понимания стиля, пишет новые тесты в блок `#[cfg(test)]` и запускает `cargo test -p cc-api`.

### 5. Работа с git

**Задача:** Начать новую фичу, закоммитить изменения и создать PR.

```
/branch
```

Агент предлагает имя ветки на основе контекста текущей задачи и создаёт её через `git checkout -b`. Удобно, чтобы не отвлекаться на именование.

```
/commit
```

Агент запускает `git diff --staged`, анализирует изменения и предлагает commit message в формате conventional commits. После подтверждения коммитит.

```
/commit-push-pr
```

Комбинированная команда: коммит → push → создание PR с автоматически сгенерированным описанием на основе diff.
...
```
> Найди все места где создаётся ApiError::Timeout и объясни в каких
  ситуациях он возникает. Покажи путь от вызова до обработки ошибки.
```

Агент использует Grep для поиска всех вхождений, читает контекст вокруг каждого, строит объяснение потока ошибки от источника до обработчика.
...
Агент использует инструмент Agent для запуска трёх субагентов параллельно. Каждый возвращает отчёт, основной агент объединяет результаты.
