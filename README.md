...
**Задача:** Проверить изменения в текущей ветке перед созданием PR.

```
/review
```

Агент запускает `git diff main`, анализирует изменения на предмет ошибок, нарушений стиля и потенциальных проблем. Возвращает структурированный список замечаний с указанием файлов и строк.
